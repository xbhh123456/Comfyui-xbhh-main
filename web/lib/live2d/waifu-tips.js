  ...
  static async initCheck(e, t = []) {
    ...
    if (s.useCDN) {
      ...
      if (s.models.length > 0) {
        if (s.modelId >= s.models.length) s.modelId = 0;
        if (s.modelTexturesId >= s.models[s.modelId].paths.length) s.modelTexturesId = 0;
      } else {
        a.error("No models found in configuration.");
      }
    }
    return s;
  }
      ...
      try {
        ...
        if (2 === o) {
          if (!this.cubism2model) {
            ...
            let retry = 0;
            const isReady = () => typeof Live2D !== "undefined" && typeof AMotion !== "undefined";
            while (!isReady() && retry < 30) {
          ...
          const { AppDelegate: t } = await import("./chunk/xbhh2.js");
  ...
  async loadModel(e) {
    ...
    if (this.useCDN) {
      ...
      if (!this.models[this.modelId]) {
        a.error(`Invalid modelId: ${this.modelId}`);
        return;
      }
      t = this.models[this.modelId].paths[this.modelTexturesId];
      if (!t) {
        a.error(`Invalid modelTexturesId: ${this.modelTexturesId}`);
        return;
      }
      s = await this.fetchWithCache(t);
    }
...
async function r(t) {
  ...
  window.xbhhModelInstance = await c.initCheck(t, l);
  (await window.xbhhModelInstance.loadModel(""),
    new d(window.xbhhModelInstance, t, o).registerTools(),
    (function () {
        const e = document.getElementById("waifu");
        if (!e) return;
        ...
        // 加载持久化位置
        const savedPos = localStorage.getItem("waifu-pos");
        if (savedPos) {
          try {
        ...
        e.addEventListener("mousedown", (n) => {
          if (0 !== n.button) return; // 仅左键拖拽
          const l = document.getElementById("live2d");
          if (n.target !== l) return;
          
          n.preventDefault();
          const startX = n.clientX - e.offsetLeft;
          const startY = n.clientY - e.offsetTop;
          
          // 移除 bottom 约束
          e.style.bottom = "auto";

          const moveHandler = (n) => {
            let left = n.clientX - startX;
            let top = n.clientY - startY;
            
            // 边界检查
            if (left < 0) left = 0; else if (left > t - o) left = t - o;
            if (top < 0) top = 0; else if (top > s - i) top = s - i;
